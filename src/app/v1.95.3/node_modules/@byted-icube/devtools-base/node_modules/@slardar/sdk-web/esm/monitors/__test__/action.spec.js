...
import { findElementWithAttributeByParents, getActionName, getActionNameByAttribute, isSupportsElementClosest, isSVGTarget, isElementWithinTwoLevels, isValidTarget, DEFAULT_ATTR_NAME, shouldIgnoreEvent, shouldIgnoreUrl, } from '../action/util';
import { mockHttpEvent, mockResourceErrorEvent, mockResourceEvent } from './utils';
...
test('shouldIgnoreUrl', function (t) {
    t.false(shouldIgnoreUrl(undefined, undefined));
    t.false(shouldIgnoreUrl(undefined, 'a'));
    t.false(shouldIgnoreUrl([], 'a'));
    t.false(shouldIgnoreUrl(['a'], ''));
    t.false(shouldIgnoreUrl(['a'], undefined));
    t.true(shouldIgnoreUrl(['a'], 'a'));
    t.true(shouldIgnoreUrl(['a', 'b'], 'a'));
    t.true(shouldIgnoreUrl(['a', 'b', /id\/\d+/], 'id/123'));
    t.false(shouldIgnoreUrl(['a', 'b', /id\/\d+/], 'id/new'));
});
test('shouldIgnoreEvent', function (t) {
    // HttpEvent
    var mockEvent = mockHttpEvent('http://example.com/new');
    t.false(shouldIgnoreEvent(undefined, mockEvent));
    t.false(shouldIgnoreEvent([], mockEvent));
    t.true(shouldIgnoreEvent(['example.com'], mockEvent));
    t.false(shouldIgnoreEvent([/example.com\/id\/\d+/], mockEvent));
    // ResourceEvent
    mockEvent = mockResourceEvent('http://example.com/id/123');
    t.true(shouldIgnoreEvent([/example.com\/id\/\d+/], mockEvent));
    t.false(shouldIgnoreEvent([/example.com\/user/], mockEvent));
    // ResourceErrorEvent
    mockEvent = mockResourceErrorEvent('http://example.com/user');
    t.true(shouldIgnoreEvent([/example.com\/user/], mockEvent));
    // Other EventType should return false
    t.false(shouldIgnoreEvent(['example.com'], { ev_type: 'pageview' }));
    t.false(shouldIgnoreEvent(['example.com'], { ev_type: 'js_error' }));
});//# sourceMappingURL=https://trae.private/sourcemaps/424b4bd987c6c6a4cadf1a08da420593cc6bf75a/node_modules/@byted-icube/devtools-base/node_modules/@slardar/sdk-web/esm/monitors/__test__/action.spec.js.map