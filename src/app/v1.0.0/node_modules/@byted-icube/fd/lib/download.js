...
const tmpDir = path.join(os.tmpdir(), `icube-fdbinary-cache-${packageVersion}`);
...
function untar(zipPath, destinationDir) {
  console.log('untar', zipPath, destinationDir)
  return new Promise((resolve, reject) => {
    const unzipProc = child_process.spawn('tar', ['--strip-components=1', '-xvf', zipPath, '-C', destinationDir], { stdio: 'inherit' });
 ...
 * @param {string} destinationDir
 */
async function unzipFd(zipPath, assertName, destinationDir) {
  if (isWindows) {
  ...
  const expectedName = path.join(destinationDir, 'fd');
  ...
  throw new Error(`Expecting fd or fd.exe unzipped into ${destinationDir}, didn't find one.`);
}
...
module.exports = async opts => {
  ...
  const assetFileName = ['fd', opts.version, opts.target].join('-');
  const assetName = assetFileName + extension;
  ...
  try {
    const destinationPath = await unzipFd(assetDownloadPath, assetFileName, opts.destDir);
    if (!isWindows) {
