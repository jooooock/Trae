...
__export(src_exports, {
  ...
  ICubeAuthClient: () => ICubeAuthClient
});
...
function save(_prefix, data, clear = false) {
  ...
  Object.keys(data).forEach((key) => {
    ...
    import_js_cookie.default.set(joinCacheKey(key), JSON.stringify(value), {
...
function clearAllCookieTokens() {
  cacheKeyWithPlatforms.forEach((it) => {
    import_js_cookie.default.remove(it);
  });
}
function get(prefix, userId) {
  ...
  try {
    ...
    for (const key of cacheKeyWithPlatforms) {
      const cookiesCache = import_js_cookie.default.get(key);
      if (cookiesCache) {
