...
import { DEFAULT_EVENT_TARGET, DEFAULT_TIME_FUNCTION, observeAsyncErrorWithSource, xmlHttpRequestProps, } from '../async_error';
...
test.serial('should report all the errors from inside to outside if we use catch global', function (t) {
    // mock env
    var window = {};
    ...
    var subject = createSafeObserverWithNext(observeAsyncErrorWithSource);
    subject.subscribe(reportStub);
    // test
    var cb = function () {
...
test.serial('should time function is hooked and capture error', function (t) {
    // mock env
    var window = {};
    ...
    var reportStub = sinon.stub();
    var subject = createSafeObserverWithNext(observeAsyncErrorWithSource);
    var unsubscribe = subject.subscribe(reportStub);
    // test
    DEFAULT_TIME_FUNCTION.forEach(function (method) {
        var cb = function () {
            throw new Error('test');
...
test.serial('should xhr is hooked and capture error when multiple xhr instances exist ', function (t) {
    // mock env
    var MockXhr = Function;
    MockXhr.prototype.send = sinon.stub();
    var reportStub = sinon.stub();
    global.XMLHttpRequest = MockXhr;
    var subject = createSafeObserverWithNext(observeAsyncErrorWithSource, reportStub);
    var unsubscribe = subject.subscribe(function () { });
    // 假设多个 xhr 实例共用一组回调函数
    var cbs = {};
    ...
    xmlHttpRequestProps.forEach(function (prop) {
        t.throws(function () { return xhr2[prop](); });
        // 取消监听后，subject 会执行 next，但是没有 Subscriber
        t.is(reportStub.callCount, 1);
        reportStub.reset();
    });
});
...
test.serial('should addEventListener is hooked and capture error', function (t) {
    // moc env
    var window = {};
    var eventBus = {};
...
test.serial('should capture error if fn.handleEvent is a function', function (t) {
    // moc env
    var window = {};
    var eventBus = {};
    DEFAULT_EVENT_TARGET.forEach(function (target) {
...
});//# sourceMappingURL=https://trae.private/sourcemaps/424b4bd987c6c6a4cadf1a08da420593cc6bf75a/node_modules/@byted-icube/devtools-base/node_modules/@slardar/sdk-web/esm/collector/__test__/async_error.spec.js.map