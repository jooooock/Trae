...
function AjaxMonitorPlugin(client) {
    client.on('init', function () {
        var config = getPluginConfig(client, AJAX_MONITOR_PLUGIN_NAME, defaultConfig$3);
        if (!config) {
            return;
        }
        var tearDownGroup = [];
...
function FetchMonitorPlugin(client) {
    client.on('init', function () {
        var config = getPluginConfig(client, FETCH_MONITOR_PLUGIN_NAME, defaultConfig$2);
        if (!config) {
            return;
        }
        var tearDownGroup = [];
...
function PageviewMonitorPlugin(client) {
    client.on('init', function () {
        var _a;
        var config = getPluginConfig(client, PAGEVIEW_MONITOR_PLUGIN_NAME, defaultConfig$1);
        if (!config || !location) {
            return;
        }
...
var PLUGINS_LOAD_PREFIX = "https:" + "//" + ("sf16-short-sg.bytedapm.com") + "/slardar/fe/sdk-web/plugins";
var SDK_VERSION = "1.12.3" ;
...
export { BATCH_REPORT_PATH, DEFAULT_IGNORE_PATHS, DEFAULT_SAMPLE_CONFIG, DEFAULT_SAMPLE_GRANULARITY, DEFAULT_SENDER_SIZE, DEVICE_ID_COOKIE_NAME, EV_METHOD_MAP, EXTRA_INDEPENDENT_PLUGINS, InjectConfigPlugin, InjectEnvPlugin, InjectQueryPlugin, ObserveErrorPlugin, PLUGINS_LOAD_PREFIX, PluginMap, PrecollectPlugin, REPORT_DOMAIN, SDK_NAME, SDK_VERSION, SETTINGS_DOMAIN, SETTINGS_PATH, STORAGE_PREFIX, TimeCalibrationPlugin, USER_ID_COOKIE_NAME, addConfigToReportEvent, addEnvToSendEvent, applyPlugin, browserBuilder, buildSelfErrorEvent, configHolder, createBrowserClient, createBrowserConfigManager, createMinimalBrowserClient, createStore, browserClient as default, doesPluginExistInRegistry, filterIfPluginDisabled, getConsumeStored, getDefaultConfig, getDefaultSessionId, getDefaultUserIdAndDeviceId, getGlobalInstance, getGlobalName, getPluginFromRegistry, getReportUrl, getServerConfig, getSettingsUrl, getStorageKey, getStoreOrConsume, glueCodeForStorageSecurity, hasSetStorageItem, loadCombinedPlugins, loadIndependentPlugins, loadNow, loadPlugins, loadPluginsOnPageLoad, mergeSampleConfig, normalizeInitConfig, normalizeUserConfig, parseServerConfig, register, reverseMap, setStorageUserIdAndDeviceId, toObservableArray, validateInitConfig };//# sourceMappingURL=https://trae.private/sourcemaps/424b4bd987c6c6a4cadf1a08da420593cc6bf75a/node_modules/@slardar/integrations/node_modules/@slardar/web/sg/index.esm.js.map
