...
import { observeDOMContentLoad, observeLoad, observeHidden, readyStateIsComplete, visibilityStateIsHidden, observeActivated, } from '../page';
...
test.serial('observeActivated - mockDocument with prerendering', function (t) {
    var nextStub = sinon.stub();
    var createMockDocument = function (prerendering) {
        return {
            prerendering: prerendering,
            addEventListener: sinon.spy(),
        };
    };
    var mockDocument = createMockDocument(true);
    // @ts-expect-error
    global.document = mockDocument;
    var subject = createSafeObserverWithNext(observeActivated);
    subject.subscribe(nextStub);
    t.true(mockDocument.addEventListener.calledOnce);
    t.is(mockDocument.addEventListener.args[0][0], 'prerenderingchange');
    t.is(mockDocument.addEventListener.args[0][2], true);
    t.false(nextStub.called);
    // Simulate the event
    var eventListener = mockDocument.addEventListener.getCall(0).args[1];
    eventListener();
    t.true(nextStub.calledOnce);
});
test.serial('observeActivated - mockDocument without prerendering', function (t) {
    var nextStub = sinon.stub();
    var createMockDocument = function (prerendering) {
        return {
            prerendering: prerendering,
    ...
    var mockDocument = createMockDocument(false);
    // @ts-expect-error
    global.document = mockDocument;
    var subject = createSafeObserverWithNext(observeActivated);
    subject.subscribe(nextStub);
    t.false(mockDocument.addEventListener.calledOnce);
    t.true(nextStub.called);
});
...
test.serial('observeHidden', function (t) {
    ...
    t.true(nextStub.args[5][0]);
    document.dispatchEvent(new window.Event('visibilitychange'));
    t.false(nextStub.args[6][0]);
    sb.restore();
});
test.serial('observeActivated with prerendering', function (t) {
    browserEnv();
    // @ts-expect-error
    ...
    var subject = createSafeObserverWithNext(observeActivated);
    subject.subscribe(nextStub);
    t.true(nextStub.notCalled);
    document.dispatchEvent(new window.Event('prerenderingchange'));
    t.true(nextStub.calledOnce);
});
test.serial('observeActivated without prerendering', function (t) {
    browserEnv();
    // @ts-expect-error
...
test.serial('observeActivated without undefined prerendering field', function (t) {
    browserEnv();
    // @ts-expect-error
    ...
    document.dispatchEvent(new window.Event('prerenderingchange'));
    t.true(nextStub.calledOnce);
});//# sourceMappingURL=https://trae.private/sourcemaps/424b4bd987c6c6a4cadf1a08da420593cc6bf75a/node_modules/@slardar/base/node_modules/@slardar/sdk-web/esm/collector/__test__/page.spec.js.map